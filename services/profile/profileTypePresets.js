 ...
 *
 * These presets define default variable values for each profile type.
 * They are used for seeding initial templates when organizations are created.
 *
 * NOTE: In production, profile templates are stored in the database and are
...
// Base preset ensures every profile type has the exact same keys (with sensible defaults).
...
const STORE_TYPE_PRESETS = {
  ...
  franchise_location: {
    ...
    pros: "Brand recognition drives customer traffic and reduces marketing needs. Proven systems and operational support provide guidance, while weather-independent operations ensure consistency. Higher growth potential than independent profiles offers scalability.",
...
/**
 * Get preset for a profile type
 * @param {string} profileType - Profile type ("food_truck", "indoor", "outdoor")
 * @returns {Object} Preset variables object
 */
function getPreset(profileType) {
  if (!profileType) {
    throw new Error("profileType is required");
  }

  const preset = STORE_TYPE_PRESETS[profileType];
  if (!preset) {
    throw new Error(
      `Invalid profileType: ${profileType}. Must be one of: ${Object.keys(STORE_TYPE_PRESETS).join(", ")}`
    );
  }
 ...
 * @returns {Array<string>} Array of profile type keys
 */
 ...
 * @param {string} profileType - Profile type to validate
 * @returns {boolean} True if valid
 */
function isValidStoreType(profileType) {
  return profileType && profileType in STORE_TYPE_PRESETS;
}
