...
const calendar = require("./classroomCalendar.service");
...
exports.getCalendarReminders = async (req, res) => {
  ...
  try {
    ...
    return res.json({ reminders: await calendar.getCalendarReminders({ classroomId, organizationId }) });
  } catch (error) {
    ...
    console.error("Failed to load calendar reminders", error.message);
    return res.status(500).json({ error: "Scheduled reminders are temporarily unavailable." });
  }
};
