...
Deployment secrets are read from the ignored root `.env` file for development and `.env.production` for production. The renderer streams the resolved JSON directly to `doctl`; it does not persist a resolved app spec.
...
Clerk values must be different between the two environment files. Add Stripe live values to `.env.production` and Stripe test values to `.env` before enabling checkout or registering the corresponding webhook endpoints.
