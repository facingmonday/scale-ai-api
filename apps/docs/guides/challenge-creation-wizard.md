...
Creation validates supplied dates again. Automatic opening requires a date at least 24 hours away. A schedule that has become too soon returns HTTP 409 with `code: WIZARD_SCHEDULE_STALE` and a refreshed proposal for review. Nothing is saved in that response. Manual opening does not require an opening date or the 24-hour lead time. Supplied dates must follow opening, deadline, closing, and processing order; delayed feedback requires a release date at or after a supplied processing date. Invalid local times in a daylight-saving gap are rejected.
