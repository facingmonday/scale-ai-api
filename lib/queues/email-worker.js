const { queues, ensureQueueReady } = require("./index");
const mongoose = require("mongoose");

// Import required services
const { sendEmail } = require("../sendGrid/sendEmail");
// Email worker now handles only email notifications

/**
 * Process email sending jobs
 */
const processEmailSending = async (job) => {
  const {
    type,
    notificationId,
    recipient,
    templateData,
    templateSlug,
    organizationId,
  } = job.data;

  let isReceipt = !!job.data.decisionReceipt;
  let canUpdateNotification = !isReceipt;
  try {
    // Connect to MongoDB if not already connected
    if (mongoose.connection.readyState !== 1) {
      await mongoose.connect(process.env.MONGO_URL || process.env.MONGO_URI);
    }

    // Get the Notification model
    const NotificationModel = require("../../services/notifications/notifications.model");

    let notification;
    let receiver;

    if (notificationId) {
      // Load notification from database
      notification = isReceipt
        ? await NotificationModel.findOne({ _id: notificationId, organization: organizationId })
        : await NotificationModel.findById(notificationId);
      if (!notification) {
        throw new Error(`Notification ${notificationId} not found`);
      }

      if (notification.challengeEmailRunId) {
        if (notification.metadata?.emailSent || notification.metadata?.emailSkipped) {
          return { success: true, alreadyProcessed: true, notificationId };
        }
        const emailService = require("../../services/challenge/lib/challengeEmailService");
        const reason = await emailService.deliveryCheck(notification);
        if (reason) return emailService.skip(notification, reason);
      }

      if (notification.decisionReceipt) {
        isReceipt = true;
        canUpdateNotification = false;
        if (!organizationId || String(notification.organization) !== String(organizationId)) {
          throw new Error("Receipt organization mismatch");
        }
        canUpdateNotification = true;
        if (notification.metadata?.emailSent || notification.metadata?.emailSkipped) {
          return { success: true, alreadyProcessed: true, notificationId };
        }
        const receipts = require("../../services/decision/lib/decisionReceiptService");
        const reason = await receipts.deliveryCheck(notification);
        if (reason) return receipts.skip(notification, reason);
      }

      // Get receiver information
      receiver = await NotificationModel.getReceiver(
        notification.recipient,
        notification.templateData,
        notification.modelData,
        organizationId || notification.organization,
        { throwOnError: isReceipt }
      );

      if (!receiver) {
        if (isReceipt) return require("../../services/decision/lib/decisionReceiptService").skip(notification, "Recipient is unavailable");
        if (notification.challengeEmailRunId) {
          return require("../../services/challenge/lib/challengeEmailService").skip(notification, "Recipient is unavailable");
        }
        throw new Error("Unable to determine notification receiver");
      }
    } else {
      // Use provided recipient data (for direct sending)
      receiver = recipient;
      notification = {
        type,
        title: job.data.title || "Notification",
        message: job.data.message || "",
        templateSlug,
        templateData,
        organization: organizationId,
      };
    }

    if (notification.challengeEmailRunId) {
      const emailService = require("../../services/challenge/lib/challengeEmailService");
      if (!NotificationModel.checkRecipientPreferences(receiver, "email")) {
        return emailService.skip(notification, "Email preference is disabled");
      }
      if (!receiver.email) return emailService.skip(notification, "Recipient email is unavailable");
      // Address resolution can involve an external request. Check eligibility again
      // immediately before delivery, and refresh title/deadline/link at that point.
      const reason = await emailService.deliveryCheck(notification);
      if (reason) return emailService.skip(notification, reason);
    }

    if (isReceipt) {
      const receipts = require("../../services/decision/lib/decisionReceiptService");
      const reason = !NotificationModel.checkRecipientPreferences(receiver, "email")
        ? "Email preference is disabled" : !receiver.email ? "Recipient email is unavailable"
        : await receipts.deliveryCheck(notification);
      if (reason) return receipts.skip(notification, reason);
    }

    let deliveryResult;

    switch (type) {
      case "email": {
        console.log(`📧 Sending email notification`);
        // Build email payload and send directly to avoid recursive enqueues
        const to = {
          email: receiver.email,
          name:
            receiver.name ||
            `${receiver.firstName || ""} ${receiver.lastName || ""}`.trim(),
        };
        const from = {
          email: process.env.SENDGRID_FROM_EMAIL || "no-reply@scaleai.com",
          name: process.env.SENDGRID_FROM_NAME || "ScaleAI",
        };

        deliveryResult = await sendEmail({
          to,
          from,
          subject: notification.title,
          html: notification.html,
          text: notification.text || notification.message,
          templateSlug: notification.templateSlug,
          templateData: notification.templateData,
          modelData: notification.modelData,
          organizationId: organizationId || notification.organization,
          populateTemplateData: !isReceipt,
        });
        break;
      }

      default:
        throw new Error(`Unknown notification type: ${type}`);
    }

    if (deliveryResult?.skipped) {
      console.log(
        `⏭️ EMAIL notification skipped: ${deliveryResult.reason}`
      );

      if (notificationId) {
        await NotificationModel.findByIdAndUpdate(notificationId, {
          "metadata.emailQueued": false,
          "metadata.emailSent": false,
          "metadata.emailSkipped": true,
          "metadata.emailSkipReason": deliveryResult.reason,
          "metadata.emailError": null,
          status: "Skipped",
        });
      }

      return {
        success: true,
        skipped: true,
        reason: deliveryResult.reason,
        type,
        notificationId,
      };
    }

    if (deliveryResult?.sent) {
      console.log(`✅ ${type.toUpperCase()} notification sent successfully`);

      // Update notification status if we have a notification ID
      if (notificationId) {
        const statusUpdate = {};
        if (type === "email") {
          statusUpdate["metadata.emailQueued"] = false;
          statusUpdate["metadata.emailSent"] = true;
          statusUpdate["metadata.emailSkipped"] = false;
          statusUpdate["metadata.emailSkipReason"] = null;
          statusUpdate["metadata.emailError"] = null;
        }
        // Reflect final delivery state on the notification
        statusUpdate.status = "Sent";

        await NotificationModel.findByIdAndUpdate(notificationId, statusUpdate);
      }

      return {
        success: true,
        type,
        notificationId,
        sentAt: new Date().toISOString(),
      };
    } else {
      throw new Error(`${type} notification returned no delivery result`);
    }
  } catch (error) {
    console.error(
      `❌ Email job failed: ${type} for notification ${notificationId}`,
      isReceipt ? "Receipt delivery failed; inspect queue status" : error
    );

    // Update only a notification belonging to this job's organization.
    if (notificationId && canUpdateNotification) {
      try {
        const NotificationModel = require("../../services/notifications/notifications.model");
        const statusUpdate = {};
        statusUpdate["metadata.emailError"] = isReceipt ? "Receipt delivery failed" : error.message;
        statusUpdate.status = "Failed";

        await NotificationModel.findByIdAndUpdate(notificationId, statusUpdate);
      } catch (updateError) {
        console.error(
          "Failed to update notification with error status:",
          updateError
        );
      }
    }

    throw isReceipt ? new Error("Receipt delivery failed") : error;
  }
};

/**
 * Initialize email sending worker
 */
const initEmailWorker = () => {
  console.log("📧 Initializing email sending worker...");

  const concurrency = Math.max(
    1,
    Number(process.env.EMAIL_WORKER_CONCURRENCY || 1)
  );

  // Process email sending jobs with concurrency
  queues.emailSending.process(
    "send-notification",
    concurrency,
    processEmailSending
  );

  // Catch-all processor for legacy/unnamed jobs
  queues.emailSending.process(concurrency, processEmailSending);

  // Handle job completion
  queues.emailSending.on("completed", (job) => {
    console.log(`✅ Email job completed: ${job.data.notificationId || job.id}`);
  });

  // Handle job failure
  queues.emailSending.on("failed", (job, err) => {
    console.error(
      `❌ Email job failed: ${job.data.notificationId || job.id} - ${err.message}`
    );
  });

  // Handle job stalled
  queues.emailSending.on("stalled", (jobId) => {
    console.warn(`⚠️ Email job stalled: ${jobId}`);
  });

  // Handle stalled job recovery
  queues.emailSending.on("stalled", async (jobId) => {
    console.log(`🔄 Attempting to recover stalled email job: ${jobId}`);
    try {
      const job = await queues.emailSending.getJob(jobId);
      if (job) {
        await job.retry();
        console.log(`✅ Retried stalled email job: ${jobId}`);
      }
    } catch (error) {
      console.error(`❌ Failed to retry stalled email job ${jobId}:`, error);
    }
  });

  // Log when worker starts processing
  queues.emailSending.on("active", (job) => {
    console.log(
      `🔄 Email worker processing job ${job.id}: ${job.data.type} for notification ${job.data.notificationId}`
    );
  });

  require("../../services/decision/lib/decisionReceiptService").startRecovery();
  console.log("✅ Email sending worker initialized");
};

/**
 * Add email sending job to queue
 */
const enqueueEmailSending = async (data) => {
  // Ensure queue connection is ready
  await ensureQueueReady(queues.emailSending, "emailSending");

  const jobData = {
    type: "email",
    ...data,
    enqueuedAt: new Date().toISOString(),
  };

  const deterministicJobId = data.notificationId
    ? `email-notification:${String(data.notificationId)}`
    : undefined;
  if (deterministicJobId) {
    const existingJob = await queues.emailSending.getJob(deterministicJobId);
    if (existingJob) {
      const state = await existingJob.getState();
      if (data.decisionReceipt) return existingJob;
      if (state === "failed") await existingJob.retry();
      if (
        state === "completed" &&
        existingJob.returnvalue?.skipped &&
        process.env.SEND_EMAIL === "true"
      ) {
        await existingJob.remove();
      } else {
        return existingJob;
      }
    }
  }

  const job = await queues.emailSending.add("send-notification", jobData, {
    ...(deterministicJobId ? { jobId: deterministicJobId } : {}),
    priority: getJobPriority(),
    delay: getJobDelayMs(),
  });

  return job;
};

/**
 * Get job priority based on type
 */
const getJobPriority = () => 3; // Medium priority for emails

/**
 * Get job delay based on type
 */
const getJobDelayMs = () =>
  Math.max(0, Number(process.env.EMAIL_JOB_DELAY_MS || 1000));

module.exports = {
  initEmailWorker,
  enqueueEmailSending,
  processEmailSending,
};
