...
const processSimulationJob = async (job) => {
  const { jobId, recalculationRunId = null } = job.data;
  if (!jobId) {
  ...
  return SimulationWorker.processJob(jobId, {
    ...
    expectedRecalculationRunId: recalculationRunId,
  });
};
...
const initSimulationWorker = () => {
  ...
  const parsedConcurrency = parseInt(
    process.env.SIMULATION_CONCURRENCY || "2",
    10
  );
  ...
  console.log(`✅ Simulation worker initialized (concurrency: ${concurrency})`);
};

/**
 * Enqueue a simulation job (adds Bull job for a SimulationJob document)
 */
const enqueueSimulationJob = async (jobId, options = {}) => {
  if (!jobId) {
  ...
  const recalculationRunId = options.recalculationRunId || null;

  return queues.simulation.add(
    { jobId, recalculationRunId },
    {
      // Deduplicate Bull jobs per SimulationJob to avoid accidental queue storms
      // (e.g., retry paths enqueuing the same SimulationJob multiple times).
      jobId: recalculationRunId
        ? `simulation:${String(jobId)}:${recalculationRunId}`
        : `simulation:${String(jobId)}`,
      attempts: 3,
