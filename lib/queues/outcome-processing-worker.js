...
const processOutcomeProcessingJob = async (job) => {
  ...
  if (challenge.isClosed) {
    if (challenge.processingRun?.preparing) {
      await require("../../services/job/lib/challengeProcessing").startChallenge(
        { challengeId, organizationId, clerkUserId },
      );
    } else if (challenge.processingRun?.id) {
      await require("../../services/job/lib/challengeProcessing").enqueuePending(
        challengeId,
      );
    }
  ...
  const useBatch = (challenge.simulationMode || "batch") === "batch";
