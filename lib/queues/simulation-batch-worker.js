...
async function processPoll(job) {
  ...
  if (status === "completed") {
    ...
    // Trigger challenge closed tasks for batch mode
    try {
      const Challenge = require("../../services/challenge/challenge.model");
      const challengeDoc = await Challenge.findById(batch.challengeId).lean();
      if (challengeDoc) {
        ...
        await AutomationTaskService.trigger("AFTER_CHALLENGE_CLOSED", {
          classroomId: challengeDoc.classroomId,
          challengeId: challengeDoc._id,
          organizationId: challengeDoc.organization,
          clerkUserId: challengeDoc.updatedBy || challengeDoc.createdBy || "system",
        });
      }
    } catch (err) {
      console.error("Error triggering AFTER_CHALLENGE_CLOSED in batch mode:", err);
    }
