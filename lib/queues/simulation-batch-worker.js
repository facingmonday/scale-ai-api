...
const { round2 } = require("../number-utils");
...
async function processPoll(job) {
  ...
  if (status === "completed") {
    ...
    // Close scenario and set batch processing status (scenario was "processing" until now)
    const scenario = await Scenario.findById(batch.scenarioId);
    if (scenario && scenario.batchProcessingStatus === "processing") {
      const clerkUserId = batch.createdBy || "system";
      await scenario.setBatchProcessingStatus("completed", clerkUserId);
    }
  ...
  if (
    ...
    if (status === "cancelled" || status === "cancelling") {
      await batch.markCancelled(`OpenAI batch ended with status: ${status}`);
    } else {
      await batch.markFailed(`OpenAI batch ended with status: ${status}`);
    }

    // Close scenario and set batch processing status
    const scenario = await Scenario.findById(batch.scenarioId);
    if (scenario && scenario.batchProcessingStatus === "processing") {
      const clerkUserId = batch.createdBy || "system";
      const batchStatus =
        status === "cancelled" || status === "cancelling"
          ...
          : status === "expired"
            ? "expired"
            : "failed";
      await scenario.setBatchProcessingStatus(batchStatus, clerkUserId);
    }
