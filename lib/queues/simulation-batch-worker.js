...
} = require("../../services/classroom/classroomReadiness.service");
...
async function writeLedgerEntryFromSnapshot(jobDoc, aiResult) {
  ...
  const studentFeedback = await LedgerEntry.generateStudentFeedback({
    ...
    decisionVariables: snapshot.decisionVariables || {},
    outcomeVariables: snapshot.outcomeVariables || {},
    outcomeNotes: snapshot.outcomeNotes || "",
    priorMetrics: snapshot.priorMetrics || {},
  ...
  const ledgerInput = {
    ...
    aiMetadata,
    calculationContext,
    suppressNotification: jobDoc.suppressNotifications === true,
  };
...
async function processSubmitLocked(job) {
  ...
  await assertClassroomReady({
    ...
    organizationId,
    operation: "process",
    // The pending jobs being submitted are expected and are not conflicts.
    ignoreCheckKeys: ["in_progress_jobs"],
  });

  const challenge =
    await require("../../services/challenge/challenge.model").findById(
      challengeId,
    );
  if (
    !challenge ||
    (job.data.processingRunId || null) !== (challenge.processingRun?.id || null)
  ) {
    return { skipped: true, reason: "Superseded calculation run" };
  }
  if (challenge.processingRun?.preparing)
    throw new Error("Challenge preparation is still active");
  const activeBatch =
    await SimulationBatch.findInProgressByScenario(challengeId);
  if (activeBatch) return { skipped: true, reason: "Batch already submitted" };
  ...
  const pendingJobs = await SimulationJob.find({
    ...
    status: "pending",
    processingRunId: job.data.processingRunId || null,
  }).sort({ createdDate: 1 });
  ...
  batch.processingRunId = job.data.processingRunId || null;
  await batch.save();
...
async function processPollLocked(job) {
  ...
  if (
    !challenge ||
    (batch.processingRunId || null) !== (challenge.processingRun?.id || null)
  ) {
    return { skipped: true, reason: "Superseded calculation run" };
  }
  ...
  if (status === "completed") {
    ...
    for (const it of items) {
      ...
      if (["completed", "failed", "cancelled"].includes(jobDoc.status)) {
      ...
      try {
        ...
        LedgerEntry.enforceCashContinuity(aiResult, {
          priorMetrics: jobDoc.calculationContextSnapshot?.priorMetrics,
        });
        ...
        if (!jobDoc.dryRun) {
          writtenEntry = await writeLedgerEntryFromSnapshot(jobDoc, aiResult);
        }

        const completedJob = await SimulationJob.findOneAndUpdate(
          { _id: jobDoc._id, status: "running" },
        ...
        jobDoc.status = completedJob.status;
        jobDoc.completedAt = completedJob.completedAt;
...
async function processSubmit(job) {
  await ensureDbConnected();
  return require("../../services/job/lib/challengeProcessing").withChallengeLock(
    job.data.challengeId,
    () => processSubmitLocked(job),
    { waitMs: 60000 },
  );
}
...
async function processPoll(job) {
  await ensureDbConnected();
  const batch = await SimulationBatch.findById(job.data.simulationBatchId);
  if (!batch) return { skipped: true, reason: "Batch no longer exists" };
  return require("../../services/job/lib/challengeProcessing").withChallengeLock(
    batch.challengeId,
    () => processPollLocked(job),
    { waitMs: 60000 },
  );
}
...
const initSimulationBatchWorker = () => {
  ...
  queues.simulationBatch.on("completed", (job, result) => {
    const isPollStillRunning =
      job.name === "poll" &&
      result?.status &&
      !["completed", "failed", "expired", "cancelled"].includes(result.status);
    const base = isPollStillRunning
      ? `⏳ SimulationBatch poll scheduled: ${job.name} (${job.id})`
      : `✅ SimulationBatch job completed: ${job.name} (${job.id})`;
...
const enqueueSimulationBatchSubmit = async ({
  ...
  return queues.simulationBatch.add(
    "submit",
    { challengeId, classroomId, organizationId, clerkUserId, processingRunId },
    {
      jobId: `batch-submit:${challengeId}:${processingRunId || "legacy"}`,
      removeOnComplete: true,
      removeOnFail: false,
      attempts: 3,
