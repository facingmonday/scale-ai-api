...
async function buildAndPersistJobPayload(jobDoc, basePrompts) {
  ...
  // Note: buildAISimulationOpenAIRequest filters variables to active definitions internally
  const { rawMessages, request } =
    await LedgerEntry.buildAISimulationOpenAIRequest(context, basePrompts);

  // Build raw variable maps
  ...
  const classroomId =
    context.scenario?.classroomId ||
    context.submission?.classroomId ||
    context.scenarioOutcome?.classroomId ||
    null;
  const filtered =
    classroomId
