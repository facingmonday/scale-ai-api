...
const { ensureMongoConnected } = require("../mongo-connection");
const {
  createLedgerCompletionRecovery,
} = require("../ledger-completion-recovery");
...
const initAutomationTaskWorker = () => {
  ...
  const recover = createLedgerCompletionRecovery({
    ...
    recoverUndeliveredEvents: async () => {
      const LedgerCompletionEvent = require("../../services/job/ledgerCompletionEvent.model");
      return LedgerCompletionEvent.recoverUndeliveredEvents();
    },
  });
  void recover();
  const recoveryTimer = setInterval(() => void recover(), 60 * 1000);
  recoveryTimer.unref?.();
};
