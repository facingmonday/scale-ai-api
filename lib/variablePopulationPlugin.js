 ...
 *   appliesTo: 'profile' // The appliesTo value for VariableDefinition ('profile', 'challenge', or 'decision')
 * });
 *
 * Legacy mode (deprecated): if you still use a per-model collection with a custom FK field,
 * pass foreignKeyField (e.g. 'profileId') and omit appliesTo in the value query.
 *
...
module.exports = function variablePopulationPlugin(schema, options) {
  ...
  // Cache to profile loaded variables on document instances
  ...
  schema.methods._loadVariables = async function () {
    ...
    // Keys with inactive definitions: exclude from orphans (don't show soft-deleted variables)
    const inactiveDefs = await VariableDefinition.find({
      classroomId,
      appliesTo,
      isActive: false,
    }).select("key");
    const inactiveKeys = new Set(inactiveDefs.map((d) => d.key));
    ...
    if (outputFormat === "valueMap") {
      ...
      definitions.forEach((def) => {
        ...
        variablesMap[def.key] =
          valuesByKey[def.key] !== undefined ? valuesByKey[def.key] : defDefault;
      });

      // Orphaned values: only include keys that have no definition at all
      // Exclude keys with inactive definitions (soft-deleted)
      (variables || []).forEach((v) => {
        if (
          variablesMap[v.variableKey] === undefined &&
          !inactiveKeys.has(v.variableKey)
        ) {
    ...
    const variablesArray = definitions.map((def) => {
      ...
      const defDefault =
        definitionObj.defaultValue !== undefined && definitionObj.defaultValue !== null
          ? definitionObj.defaultValue
          : null;
      return {
        ...definitionObj,
        value:
    ...
    (variables || []).forEach((v) => {
      if (
        !definitionKeys.has(v.variableKey) &&
        !inactiveKeys.has(v.variableKey)
      ) {
        variablesArray.push({
          key: v.variableKey,
  ...
  schema.statics.populateVariablesForMany = async function (documents) {
    ...
    const inactiveDefs = await VariableDefinition.find({
      ...
      appliesTo,
      isActive: false,
    }).select("classroomId key");
    const inactiveKeysByClassroom = {};
    inactiveDefs.forEach((d) => {
      const cid = d.classroomId.toString();
      if (!inactiveKeysByClassroom[cid]) inactiveKeysByClassroom[cid] = new Set();
      inactiveKeysByClassroom[cid].add(d.key);
    });
    ...
    documents.forEach((doc) => {
      ...
      const definitionsForClassroom =
        definitionsByClassroomAndKey[classroomIdStr];
      const inactiveKeys = inactiveKeysByClassroom[classroomIdStr] || new Set();
      ...
      if (outputFormat === "valueMap") {
        ...
        definitionKeys.forEach((key) => {
          const def = definitionsForClassroom[key] || {};
          const defDefault =
            def.defaultValue !== undefined && def.defaultValue !== null
              ? def.defaultValue
              : null;
          variablesMap[key] =
            docValues[key] !== undefined ? docValues[key] : defDefault;
        });

        // Orphaned values: only include keys with no definition at all
        // Exclude keys with inactive definitions (soft-deleted)
        Object.keys(docValues).forEach((key) => {
          if (variablesMap[key] === undefined && !inactiveKeys.has(key)) {
      ...
      const variablesArray = definitionKeys.map((key) => {
        ...
        const defDefault =
          definition && definition.defaultValue !== undefined && definition.defaultValue !== null
            ? definition.defaultValue
            : null;
        return {
          ...definition,
          value: docValues[key] !== undefined ? docValues[key] : defDefault,
        };
      });
      ...
      // Exclude keys with inactive definitions (soft-deleted)
      Object.keys(docValues).forEach((key) => {
        if (!definitionsForClassroom[key] && !inactiveKeys.has(key)) {
          variablesArray.push({
            key,
            value: docValues[key],
          });
