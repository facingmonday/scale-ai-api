...
function ChallengeClosedEmail(props) {
  const {
    challenge = {},
    classroom = {},
    member = {},
  ...
  const scenarioLink =
    link ||
    challenge?.link ||
    `${host}/class/${encodeURIComponent(classroomId)}/challenge/${encodeURIComponent(challengeId)}`;

  const scenarioTitle = challenge?.title || "Challenge results";
  const classroomName = classroom?.name || "your class";
  const memberName = member?.firstName || member?.name || "Student";
  const organizationName = organization?.name;
  const previewText = `Challenge Results Available - ${scenarioTitle}`;
        ...
        >
          ...
          >
            ...
            <Section style={{ padding: "30px" }}>
              ...
              >
                Results are now available for a challenge in{" "}
...
module.exports = {
  ChallengeClosedEmail,
  populatePaths: ["challenge", "classroom", "member", "ledger"],
};
